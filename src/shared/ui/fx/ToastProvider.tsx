  ...
  const api: ToastApi = useMemo(
    () => ({
      ...
      success: (title, description) =>
        show({ title, variant: 'success', ...(description ? { description } : {}) }),
      info: (title, description) =>
        show({ title, variant: 'info', ...(description ? { description } : {}) }),
      warning: (title, description) =>
        show({ title, variant: 'warning', ...(description ? { description } : {}) }),
      error: (title, description) =>
        show({ title, variant: 'error', ...(description ? { description } : {}) }),
    }),
    [show],
  );
...
function ToastCard({ t, onClose }: { t: ToastItem; onClose: () => void }) {
  ...
  return (
    ...
    >
      <div className="flex items-start gap-3 p-4">
        ...
        <div className="min-w-0 flex-1">
          ...
          {actionLabel && onAction && (
            <button
              onClick={onAction}
              className="mt-3 text-sm underline underline-offset-4 hover:text-primary"
            >
              {actionLabel}
            </button>
          )}
      ...
      <div
        aria-hidden
        className="h-0.5 w-full bg-[linear-gradient(90deg,transparent,var(--color-primary),transparent)]"
      />
    </motion.div>
  );
}
