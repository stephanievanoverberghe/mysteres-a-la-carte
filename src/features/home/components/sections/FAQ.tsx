...
const FAQS: QA[] = [
  ...
  },
  {
    ...
    a: (
      <>
        Oui. Sélectionnez le menu adapté (ex. <b>Botanique</b> végétarien) et précisez vos
        restrictions dans le champ “Allergies”. Nous vous confirmerons les options possibles.
      </>
    ),
  },
  {
        ...
        <div className="md:mx-8 lg:mx-0">
          ...
          <div className="mt-10 flex flex-col items-stretch gap-3 md:flex-row md:items-center md:justify-between text-sm">
            ...
            <span className="text-muted-foreground text-center md:text-left">
              Toujours une question&nbsp;? On vous répond rapidement.
            </span>
          </div>
        </div>
      </div>
...
function FAQItem({ item, open, onToggle }: { item: QA; open: boolean; onToggle: () => void }) {
  ...
  return (
    <article className="relative rounded-2xl border border-muted bg-background/60 p-4 md:p-5 shadow-soft">
      ...
      <span
        aria-hidden
        className="pointer-events-none absolute left-4 right-4 top-0 h-0.5 bg-[linear-gradient(90deg,transparent,var(--color-primary),transparent)]"
      />
      ...
      >
        ...
        <ChevronDown
          className={`h-5 w-5 shrink-0 transition-transform ${open ? 'rotate-180 text-primary' : 'rotate-0 text-muted-foreground'}`}
        />
      </button>
